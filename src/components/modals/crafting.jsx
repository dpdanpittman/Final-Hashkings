...
function CraftingModal(props) {
    const extractXPBoosters = (boostersObj, range) => Object.keys(boostersObj).slice(range[0], range[1]).map(booster => (
        <div className="item">
            <img className="booster-image" src={ boostersObj[booster].image } alt={`XP Booster ${booster}`} />
            <div className="image-caption-wrapper">
                <div className="image-caption">
                    <span>
                        <img className="buds" src={ BudsPNG } alt="Buds" /> <b>{ boostersObj[booster].buds }</b>
                    </span>
                    <span>
                    <img className="xp" src={ XpPNG } alt="Buds" /> <b>{ boostersObj[booster].boost }</b>
                    </span>
                    <span>
                        <img className="crafting-icon highlight-on-hover" src={ CraftingPNG } alt="Click to craft" />
                    </span>
                </div>
            </div>
        </div>
    ...
    const extractTimeBoosters = (boostersObj, range) => {
        ...
        return Object.keys(boostersObj).slice(range[0], range[1]).map(booster => (
            <div className="item">
                <img className="booster-image" src={ images[booster] } alt={`Time Booster ${booster}`} />
                <div className="image-caption-wrapper">
                    <div className="image-caption">
                        <span>
                            <img className="buds" src={ BudsPNG } alt="Buds" /> <b>{ boostersObj[booster].buds }</b>
    ...
    return (
        <>
        <Modal show={ props.show } onHide={ () => props.hideModal('crafting') } size={ props.size || null } centered>
            <div id="crafting-modal" className="base-modal pb-5">
                <div className="w-100 d-flex flex-row justify-content-center" style={{ width: "100%" }}>
                    <span className="modal-title">CRAFT</span>
                </div>
                <div className="d-flex flex-row justify-content-around images-container">
                    <div className="item">
                        <img onClick={ () => toggleSubModal("timeboosters") } className="highlight-on-hover" src={ TimeBoosterLvl1PNG } alt="Time Boosters" />
                        <div>
                            <h3>Time Boosters</h3>
                        </div>
                    </div>
                    <div className="item xp">
                        <img onClick={ () => toggleSubModal("xpboosters") } className="highlight-on-hover xp-boosters-main-img" src={ xp_boosters.BroncePNG.image } alt="XP Boosters" />
                        <div>
                            <h3>Consumables</h3>
                        </div>
                    </div>
            ...
            <div id="timeboosters" className="crafting-sub-modal">
                <i style={{ borderRadius: '50%' }} onClick={ () => toggleSubModal("timeboosters") } className="fa fa-close fa-2x text-black-50 highlight-on-hover"></i>
            ...
            <div id="xpboosters" className="crafting-sub-modal">
                <i style={{ borderRadius: '50%' }} onClick={ () => toggleSubModal("xpboosters") } className="fa fa-close fa-2x text-black-50 highlight-on-hover"></i>
                <div className="d-flex flex-row justify-content-around">
...
const mapStateToProps = state => {
    ...
    const { stats } = bucket;
    const { timeBooster } = stats.prices;
    return {
        timeBooster
    }
}
